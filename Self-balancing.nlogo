...
to recolor
  ...
  [ set color red]
   ask turtles with [distanced? and not infected?]
  [ set color blue]
  ask turtles with [not distanced? and not infected?]
  [ set color white]
...
to calculalate-cumulative-output
  let y ((count turtles with [infected? and not dead?] * .5) + (count turtles with [not infected? and not distanced? and not dead?] * 2) + (count turtles with [not infected? and distanced? and not dead?]))
